...
/**
 * ReportFormDialog — the heavy, lazily-loaded BODY of the report affordance (split out
 * of `report-dialog.tsx` in 36-04 for the public First-Load-JS budget, D-22/D-25).
 *
 * WHY ITS OWN MODULE (bundle split — load-bearing): `ReportDialog` (the muted footer
 * trigger) is a client component referenced by EVERY template footer, so Next hoists it
 * into the `/[username]` route's SHARED first-load chunk. When the heavy form + the
 * reused `TurnstileWidget` lived in the same module, that ~13 kB rode along into first
 * load even though the dialog is opened rarely. Registering the 5th template (`atelier`)
 * tipped the already-at-ceiling public bundle over the ≤200 kB invariant. Extracting the
 * form here and `dynamic()`-importing it from the trigger (loads on first open) keeps the
 * heavy code OUT of first load for every public route. `npm run check:bundle` is the
 * regression catch.
 *
 * Everything else is UNCHANGED from the original `report-dialog.tsx` body: the
 * focus-trapped MODAL a11y + motion contract (mirrors `project-modal.tsx`), the
 * Turnstile-gated submit to `/api/report` (the sole service-role writer, D-16; client
 * parse = UX, server re-parse = the gate), the generic-for-400/429/500 error copy (D-04),
 * and the two-layer token-only styling (DEF-07-03-01 — every visually-template-specific
 * value is a single `.tmpl-*` complete-value token, so each template paints from its OWN
 * scoped tokens; the island is hex-free).
 */
import { useEffect, useId, useRef, useState } from 'react';

import dynamic from 'next/dynamic';

// Lazily-loaded — shares the single `@marsidev/react-turnstile` async chunk with
// `contact-form.tsx` (kept off first load; this whole dialog body already loads on open).
 ...
 */
export function ReportFormDialog({ portfolioId, onClose }: ReportFormDialogProps) {
  const titleId = useId();
  const introId = useId();

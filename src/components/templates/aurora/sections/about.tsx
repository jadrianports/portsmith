/**
 * About section (aurora section 2) — the rosy intro spread (translated from
 * `marketing-girl/src/components/About.tsx`). Mirrors the FROZEN `SectionProps`
 * contract + `present()` + content cast + null-guard + hide-if-empty EXACTLY. `index.tsx`
 * wraps this in `<ScrollReveal as="section">`, so this renders the INNER content.
 *
 * RENDER CONTRACT: a mono kicker + heading, then a 2-col spread on desktop (avatar LEFT
 * in a soft rounded frame, bio RIGHT), stacked on mobile. `about.avatar` renders ONLY
 * when present (with its REQUIRED `avatar_alt`), via `next/image` `unoptimized` (the
 * project does client-side WebP, no server image processing — CLAUDE.md). `about.skills`
 * (flat `string[]`) is NOT rendered here — superseded by the Skills section (D-09).
 *
        ...
        {showAvatar ? (
          <div
            style={{
              ...
              boxShadow: '0 18px 44px -28px rgba(214, 51, 108, 0.35)',
            }}
          >
            <Image

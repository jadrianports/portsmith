/**
 * The SHARED, FROZEN section prop contract for the `aurora` template (11-04 Wave-C).
 * Mirrors `minimal/sections/types.ts` + `editorial/sections/types.ts` VERBATIM — the
 * prop signatures are part of the engine contract, NOT a per-template choice. Re-created
 * here (rather than imported from a sibling template) so the aurora tree is
 * self-contained (D-17) and never reaches into `minimal/` or `editorial/`.
 *
 * TWO content shapes, two prop types:
 *   • The 12 BY-TYPE sections aurora supports (Hero, About, Education, Experience,
 *     Metrics, Projects, Services, Skills, Testimonials, Moodboard, Certifications,
 *     Contact) receive a single resolved `section` row from `data.sections` — typed
 *     `SectionProps`. The row may be `undefined` when the portfolio has no section of
 *     that `type` (each section owns its own hide-if-empty logic and returns `null`).
 ...
 * (e.g. `HeroContent`, `MetricsContent`, `ServicesContent`, …) — the content already

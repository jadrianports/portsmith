/**
 * Moodboard section (atelier) — a near-verbatim mirror of `aurora/sections/moodboard.tsx`,
 * re-styled to the export's dark-editorial scoped tokens (sharp corners, acid accent). A
 * captioned image gallery + an optional brand palette. A FIRST-CLASS mapped soft-enum type
 * (`moodboard` is in `sectionContentSchemas`). `index.tsx` wraps this in `<ScrollReveal
 ...
 *
 * RENDER CONTRACT: a kicker + heading + optional subheading, then a responsive gallery of
 * captioned images (each renders ONLY when a SAFE Storage-origin image + its required alt
 * are present — `isHttpImageSrc`), then an optional brand palette row of swatches. The
 * cropped 4/3 tile is INTENTIONAL here — the moodboard is curated tiles (distinct from the
 * no-crop gallery). The palette `color` is a validated hex literal (NOT a free string), so
 * it is safe as an inline `background-color` — schema guarantees `#RGB`/`#RRGGBB`.
 *
 * COLOR: no hardcoded hex for UI chrome — every UI value reads a scoped `var(--token)`.
 * The ONLY inline color values are the validated per-swatch palette hexes.
 */

/**
 * scripts/seed-editorial-demo.ts — the idempotent `editorial`-template DEMO-portfolio
 * seed (Plan 23-01, D-15). A near-clone of `scripts/seed-aurora-demo.ts`; only the
 * persona, the template (`editorial` vs `aurora`), the section set (dev-flavored vs
 * marketer), and the log tag differ.
 *
 * WHAT IT DOES: populates a published DEMO developer portfolio on the `editorial`
 * ("Newsprint") template — `profiles` (published) + `portfolios` (on editorial) +
 * `portfolio_settings` + `sections` — from the typed `EDITORIAL_DEMO` fixture in
 * `./seed/editorial-content`. Production launch verification (LAUNCH-02 Lighthouse +
 * LAUNCH-08 public smoke) needs a published, FULLY-RENDERING portfolio PER live template;
 * this seed creates the `editorial` one at `/lena-voss` (the username comes from the
 * shared `DEMO_USERNAMES` constant, so the LHCI config + prod smoke spec import the SAME
 * value).
 *
 * IDEMPOTENT: every write is an UPSERT on a natural key (profiles by id, portfolios on
 * user_id with `template_id`=editorial set ON INSERT ONLY, portfolio_settings on
 ...
 *
 * editorial MAY be PUBLIC: `editorial`'s `visibility` is `public` (no grant needed). The
 * restricted-template grant branch below is CONDITIONAL on `template.visibility ===
 * 'restricted'`, so it is a clean no-op for a public `editorial` and self-heals a grant if
 * `editorial` were ever made restricted. The PUBLIC read path renders any portfolio
 ...
 *
 * CONTENT: comes from the GITIGNORED `scripts/seed/editorial-content.ts` (the tweakable
 * demo copy; copied from the committed `editorial-content.example.ts` template). This file
 * does NOT redefine content inline.
 *
 * USAGE:
 *   Local:  npm run seed:editorial                          (URL is localhost → no flag needed)
 *   Prod:   SEED_TARGET=prod npm run seed:editorial         (or `npm run seed:editorial -- --confirm-prod`)
 */
import { createClient } from '@supabase/supabase-js';
import { profileSchema, settingsSchema, validateSectionContent } from '@/lib/validations';
import { EDITORIAL_DEMO } from './seed/editorial-content';

// Load .env.local so `npm run seed:editorial` works without manual `export`s.
...
function fail(message: string): never {
  console.error(`[seed-editorial] ERROR: ${message}`);
  process.exit(1);
}

function log(message: string): void {
  console.log(`[seed-editorial] ${message}`);
}
 ...
 *
 * Content is taken from the EDITORIAL_DEMO fixture; the contact section surfaces the
 * intended-public `email_public` INTO its content (the SAME additive idiom the founder /
 * aurora seeds use) so editorial's contact section can render a mailto fallback under the
...
function buildSections(): { type: string; content: unknown; visible: boolean }[] {
  const s = EDITORIAL_DEMO.sections;
  ...
  const heroContent = {
    ...s.hero,
    resume_url: EDITORIAL_DEMO.profile.resume_url,
  };
  return [
...
async function main(): Promise<void> {
  ...
  if (!isLocalTarget(supabaseUrl) && !confirmProd) {
    fail(
      `Refusing to seed a NON-LOCAL target (${supabaseUrl}) without explicit ` +
        ...
        '`SEED_TARGET=prod` (or `npm run seed:editorial -- --confirm-prod`) only if ' +
  ...
  if (!profile) {
    ...
    const { data: created, error: createError } = await admin.auth.admin.createUser({
      email: EDITORIAL_DEMO.bootstrap.email,
      password: EDITORIAL_DEMO.bootstrap.password,
      email_confirm: true,
      user_metadata: {
        username,
        display_name: EDITORIAL_DEMO.profile.display_name,
      },
    });
  ...
  try {
    validatedProfile = profileSchema.parse({
      username,
      display_name: EDITORIAL_DEMO.profile.display_name,
      headline: EDITORIAL_DEMO.profile.headline,
      avatar_url: EDITORIAL_DEMO.profile.avatar_url ?? '',
    });
  } catch (err) {
    fail(
  ...
  // `published: true` is the sanctioned service-role write — REQUIRED or /lena-voss 404s.
  ...
  // --- 3. Resolve the `editorial` template id (FK target for the portfolio). -----
  ...
  if (!template) {
    fail(
      'the `editorial` template row is missing. It is seeded by migration 008 — ' +
        'apply migrations (`supabase db reset`) and re-run.',
    );
  }
  log(`editorial template resolved: ${template.id} (visibility=${template.visibility}).`);

  // --- 4. Ensure the portfolio (UNIQUE on user_id → idempotent). --------------
  // PRESERVE THE LIVE TEMPLATE CHOICE: a blind UPSERT that always SET
  // `template_id = editorial` would revert any future template the owner picks every
  // re-run. So we set `template_id = editorial` ONLY when CREATING the portfolio; when it
  ...
  if (existingPortfolio) {
    ...
    // Fresh — create on the editorial template (the demo's whole point is to be ON editorial).
    ...
    log(`portfolio created: ${portfolioId} (template_id=editorial).`);
  }

  // --- 4b. Self-healing demo-user→editorial grant (only if editorial is RESTRICTED). --
  // RESTRICTED BRANCH: conditional on `template.visibility === 'restricted'`. `editorial`
  ...
  if (template.visibility === 'restricted') {
    ...
    if (grantError) {
      fail(`demo-user→editorial template_grants upsert failed: ${grantError.message}`);
    }
    log('demo-user→editorial template_grants upserted (editorial is restricted; self-healing).');
  } else {
    // PUBLIC BRANCH: editorial is public → no grant needed.
    log(`editorial is ${template.visibility} (not restricted) — no template_grants row needed.`);
  }
  ...
  log(
    `SUCCESS: editorial demo portfolio seeded for "${username}" (${userId}) — published, ` +
      `on editorial, ${sections.length} dev sections. Reachable at /${username}. ` +

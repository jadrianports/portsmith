 ...
 *
 * Each bucket: seed exactly CAP rows in the last hour for a fresh subject, assert the
 * REAL `countAndRecord` (imported from `@/lib/rate-limit/ledger`, the shipped function
 * the auth actions call) returns `false` at CAP (the CAP+1th is over cap) and `true`
 * for a never-seen subject. Binding the assertion to the real function — not a local
 * re-implementation (WR-02) — is what makes this the binding proof that the new buckets
 * behave like the proven contact/report caps.
 */
...
describe('auth rate-limit — the per-bucket cap fires against the live ledger (HARD-04/D-11)', () => {
  for (const { bucket, cap } of BUCKETS) {
    ...
    it(`${bucket}: countAndRecord returns false at CAP=${cap} (over cap, not recorded)`, async () => {
      ...
      const allowed = await countAndRecord(bucket, subjectFor(bucket), WINDOW_MS, cap);
    ...
    it(`${bucket}: a fresh (never-seen) subject is allowed and recorded`, async () => {
      const allowed = await countAndRecord(bucket, subjectFor(bucket, true), WINDOW_MS, cap);

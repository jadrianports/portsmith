...
/** Pull the redirect target path (+ query) out of the handler's response. */
...
describe('/auth/confirm — open-redirect hardening for `next` (email branch)', () => {
  ...
  it('rejects an off-origin protocol-relative next and falls back to /dashboard', async () => {
    ...
    const loc = locationOf(res);
    // The redirect is always built from the request's own origin (never the
    // attacker host) with the query stripped — so the host is a same-origin dev
    // host and the path falls back to /dashboard rather than navigating off-origin.
    expect(loc.host).not.toContain('evil.com');
